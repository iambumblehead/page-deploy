...
var UserOptions = module.exports = (function () {
  ...
  return {
    getNew : function (spec) {
      ...
      if (spec.inputDir) {
        that.inputDir = path.normalize(spec.inputDir);       
      }

      if (spec.publicPath) {
        that.publicPath = path.normalize(spec.publicPath);       
      }

      if (spec.outputDir) {
        that.outputDir = path.normalize(spec.outputDir);       
      }

      if (spec.supportDir) {
        that.supportDir = path.normalize(spec.supportDir);       
      }
